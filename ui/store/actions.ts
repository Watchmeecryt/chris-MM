...
export async function getTransactionById(
  ...
  return await submitRequestToBackground<TransactionMeta | undefined>(
    'getTransaction',
    [id],
  );
}
...
/** ERC-7984 confidential token: `confidentialBalanceOf` handle (bytes32 hex). */
export async function confidentialErc7984GetBalanceHandle(
  chainIdHex: Hex,
  tokenAddress: string,
  userAddress: string,
): Promise<string> {
  return await submitRequestToBackground<string>(
    ...
    [chainIdHex, tokenAddress, userAddress],
  );
}

/** ERC-7984 flows: read-only `eth_call` (e.g. allowance). */
export async function confidentialErc7984EthCall(
  chainIdHex: Hex,
  to: string,
  data: string,
): Promise<string> {
  return await submitRequestToBackground<string>('confidentialErc7984EthCall', [
    chainIdHex,
    to,
    data,
  ]);
...
/** ERC-7984 unwrap finalize: `eth_getTransactionReceipt` (null while pending). */
export async function confidentialErc7984GetTransactionReceipt(
  chainIdHex: Hex,
  txHash: string,
): Promise<Record<string, unknown> | null> {
  return await submitRequestToBackground<Record<string, unknown> | null>(
...
/**
 * When tx meta has no `hash` but the unwrap already mined (e.g. stuck "Signing"), find hash on-chain.
 */
export async function confidentialErc7984FindPublishedTransactionHash(
  chainIdHex: Hex,
  fromAddress: string,
  toAddress: string,
  nonce: string,
  ...
  return await submitRequestToBackground<string | null>(
    ...
    [chainIdHex, fromAddress, toAddress, nonce],
  );
}

/**
 * ERC-7984 / Zama user-decrypt: sign canonical EIP-712 JSON (opens confirmation).
 */
export async function confidentialErc7984SignTypedDataV4(
  chainIdHex: Hex,
  fromAddress: string,
  typedDataJsonString: string,
): Promise<string> {
  return await submitRequestToBackground<string>(
    'confidentialErc7984SignTypedDataV4',
    [chainIdHex, fromAddress, typedDataJsonString],
  );
}
