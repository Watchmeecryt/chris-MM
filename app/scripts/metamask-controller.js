...
export default class MetamaskController extends EventEmitter {
  ...
  getApi() {
    ...
    return {
      ...
      confidentialErc7984GetBalanceHandle:
        this.confidentialErc7984GetBalanceHandle.bind(this),
      confidentialErc7984EthCall:
        this.confidentialErc7984EthCall.bind(this),
      confidentialErc7984GetTransactionReceipt:
        this.confidentialErc7984GetTransactionReceipt.bind(this),
      confidentialErc7984FindPublishedTransactionHash:
        this.confidentialErc7984FindPublishedTransactionHash.bind(this),
      confidentialErc7984SignTypedDataV4:
        this.confidentialErc7984SignTypedDataV4.bind(this),
  ...
  /**
   * ERC-7984 confidential token: read encrypted balance handle via `eth_call`.
   *
   ...
   */
  async confidentialErc7984GetBalanceHandle(chainIdHex, tokenAddress, userAddress) {
    const networkClientId =
      this.networkController.findNetworkClientIdByChainId(chainIdHex);
    if (!networkClientId) {
      throw new Error(
        `No network client for chain ${chainIdHex}; add or enable the network.`,
      );
    }
    const networkClient =
    ...
    const data = iface.encodeFunctionData('confidentialBalanceOf', [
      userAddress,
    ]);
    const result = await networkClient.provider.request({
      method: 'eth_call',
    ...
    const [handleHex] = iface.decodeFunctionResult(
  ...
  /**
   * ERC-7984 helper: raw `eth_call` on a chain (e.g. ERC-20 `allowance`).
   *
   * @param {`0x${string}`} chainIdHex
   * @param {`0x${string}`} to
   * @param {`0x${string}`} data
   * @returns {Promise<string>} hex return data
   */
  async confidentialErc7984EthCall(chainIdHex, to, data) {
    const networkClientId =
      this.networkController.findNetworkClientIdByChainId(chainIdHex);
    if (!networkClientId) {
      throw new Error(
  ...
  /**
   * ERC-7984 unwrap finalize: fetch a transaction receipt via `eth_getTransactionReceipt`.
   *
   * @param {`0x${string}`} chainIdHex
   * @param {`0x${string}`} txHash
   * @returns {Promise<object | null>} receipt JSON or null if still pending
   */
  async confidentialErc7984GetTransactionReceipt(chainIdHex, txHash) {
    const networkClientId =
      this.networkController.findNetworkClientIdByChainId(chainIdHex);
    if (!networkClientId) {
      throw new Error(
   ...
   * @param {string} toAddress - confidential token contract
   * @param {string} nonceRaw - hex (`0x..`) or decimal string from `txParams.nonce`
   * @returns {Promise<`0x${string}` | null>}
   */
  async confidentialErc7984FindPublishedTransactionHash(
    chainIdHex,
    fromAddress,
    toAddress,
    nonceRaw,
    ...
    const toLc = toAddress.toLowerCase();

    let nonceInt;
    if (typeof nonceRaw === 'string' && nonceRaw.startsWith('0x')) {
      nonceInt = parseInt(nonceRaw, 16);
    } else if (typeof nonceRaw === 'string' && /^\d+$/.test(nonceRaw)) {
      nonceInt = parseInt(nonceRaw, 10);
    } else {
      nonceInt = NaN;
    }
    if (!Number.isFinite(nonceInt)) {
      return null;
    }
    ...
    const maxDepth = 128n;
    for (let i = 0n; i < maxDepth && latest >= i; i++) {
      ...
      for (const tx of block.transactions) {
        ...
        let txNonce = NaN;
        if (typeof tx.nonce === 'string' && tx.nonce.startsWith('0x')) {
          txNonce = parseInt(tx.nonce, 16);
  ...
  /**
   * Prompts the user to sign EIP-712 for Zama user-decrypt (relayer complete).
   *
   * @param {`0x${string}`} chainIdHex
   * @param {`0x${string}`} fromAddress
   * @param {string} typedDataJsonString - canonical JSON (see `canonicalStringify`)
   * @returns {Promise<string>} hex signature
   */
  async confidentialErc7984SignTypedDataV4(
    chainIdHex,
    fromAddress,
    typedDataJsonString,
  ) {
    ...
    const originalRequest = {
      ...
      params: [fromAddress, typedDataJsonString],
